...
- LMmaster는 6시간마다 GitHub Releases를 자동 점검해요. 새 버전이 나오면 **한국어 toast**로 알려드려요.
- 동의 버튼만 누르면 다운로드 + minisign 서명 검증 + 설치 + 재시작까지 자동.
- 첫 설치만 위 가이드대로 받으시면, 그 후 모든 버전은 알아서 갱신돼요.
- 자동 업데이트가 싫으시면 Settings → 자동 갱신 토글로 끌 수 있어요.

#### 자주 묻는 질문

- **Q. "Windows에서 PC 보호" 또는 "확인되지 않은 게시자" 경고가 나와요.**
  - A. 코드 서명 인증서를 아직 발급 안 했어요(연 $150~$300). v1 베타 단계라 경고가 정상이에요. *추가 정보* → *실행*을 누르시면 안전하게 사용할 수 있어요. 평판이 쌓이면 경고가 사라져요.
- **Q. macOS에서 "확인되지 않은 개발자" 경고가 나와요.**
  - A. Apple Developer Program 등록을 미루고 있어요(연 $99). 우클릭 → 열기로 한 번만 통과하면 다음 실행부터 OK.
- **Q. 모델 다운로드는 어디서 해요?**
  - A. 앱 안 *모델 카탈로그*에서 받아요. Ollama / LM Studio가 자동 설치돼서 모델만 고르면 끝이에요. HuggingFace 직접 검색도 됩니다.
- **Q. 외부 서버로 데이터가 나가나요?**
  - A. 추론은 100% 로컬이에요. 외부 호출은 *GitHub Releases* (자동 업데이트) + *huggingface.co* (모델 다운로드) + *jsdelivr.net* (카탈로그 갱신) 화이트리스트만. 사용자 명시 동의 없는 외부 송신 0.
