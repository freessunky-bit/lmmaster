...
- **Crates**: 22 / **ADR**: **38** (0001~0047) / **결정 노트**: 38+
...
**남은 v2 옵션 페이즈 (즉시 ship 차단 X — v1.x로 충분)**:
- **13'.h.2.b/c — llama.cpp server 자동 spawn + mmproj sidecar** (v2 마이그레이션, 8-10h). Ollama vision + LM Studio vision 두 어댑터로 일반 사용자 vision 흐름이 작동하므로 v2로 deferred.
