...
- **Crates**: 24 / **ADR**: **53** (0001~0057) / **결정 노트**: 44+
...
- **2026-05-04 분리 sub-phase #38 + #31 머지** (R-A.3 / R-B.2 분리분 모두 완결). ① **#38 (commit `964ded0`) knowledge-stack SQLCipher caller wiring**: KnowledgeStorePool에 `with_passphrase(passphrase: String)` 생성자 + `passphrase: Option<String>` 필드 추가. `get_or_open`이 passphrase 있으면 `KnowledgeStore::open_with_passphrase`, 없으면 평문 `open`. `provision_knowledge_passphrase()` 신규 — keyring service `lmmaster` / username `knowledge-secret` 별개 entry (key-manager의 `keymanager-secret`와 격리). `provision_knowledge_store_pool()` lib.rs setup에서 호출 — keyring 미접근 시 평문 폴백 (warn 로그). sqlcipher feature OFF 빌드는 PRAGMA key 무시 → 회귀 0. 2 신규 invariant. ② **#31 (commit `8dfc116`) Knowledge IPC store_path boundary 검증**: `KnowledgeApiError::PathDenied { reason }` variant + `validate_store_path(data_dir, requested)` 헬퍼 (R-A.2 portable boundary 패턴 차용). 빈/`..`/제어 문자 거부 + 절대경로면 canonicalize prefix 검증. `validate_against_app_data_dir(app, requested)` IPC 진입점 헬퍼. 3 IPC(ingest_path / search_knowledge / knowledge_workspace_stats)에 AppHandle + validate wiring. 7 신규 invariant (empty / 정상 / .. 거부 / 제어 문자 거부 / 절대 외부 거부 / `subdir/../../escape` 거부 / kebab-case kind 직렬화). 선택적 v2.x 후속: selected_path_token (dialog plugin 미설치). **검증**: cargo workspace clippy clean / fmt clean / ACL drift 0 / 회귀 0건. **GPT Pro 30-issue 검수 + 분리 sub-phase 모두 종결.** 다음 standby = **v0.0.1 release tag push** (사용자 결정).
- **2026-05-04 Phase R-E — Architecture Cleanup 머지** (ADR-0057 채택). GPT Pro 30-issue 중 POST v0.0.1 architecture cleanup 7건 일괄 완료. **사용자 가시 동작 변경 0** (회귀 0건). ① **R-E.2 (C2, commit `34c3cf7`)** 신규 crate `openai-compat-dto` — 8 DTO 추출 + 6 invariant. adapter-lmstudio + adapter-llama-cpp inline 정의 제거 (alias import 백워드 호환). ② **R-E.5 (P1, commit `e421f24`)** `KnowledgeStorePool` 도입 — IPC 호출당 SQLite open 반복 → Arc 캐시 + FIFO eviction max=4. 4 IPC 갱신(ingest_path/search_knowledge/_with_embedder/workspace_stats) + 5 invariant. ③ **R-E.1 (T3, commit `8f2e8f9`)** raw `tokio::TcpListener` 기반 chat_stream graceful disconnect 자동 회귀 가드 — 3 어댑터(ollama/lmstudio/llama-cpp) × 2 케이스(delta 1+ → Completed / delta 0 → Failed) = 6 신규 invariant. R-C.2 fix 회귀 보호. ④ **R-E.3 (A1, commit `59b8a0a`)** 신규 crate `chat-protocol` — ChatMessage/ChatEvent/ChatOutcome 추출. adapter-ollama는 re-export로 백워드 호환. 7 invariant (None images skip + 백워드 호환 + serde tag). ⑤ **R-E.4 (A2 재스코프, commit `e102eee`)** adapter-ollama 역의존 완전 제거. 두 어댑터 + desktop이 chat-protocol 직접 import. RuntimeAdapter trait split은 ROI 낮아 보류 (현 trait이 이미 lifecycle-focused). 의존 그래프 정상화. ⑥ **R-E.6 (P4, commit `cd4fc71`)** Channel close → backend cancel cascade. chat (Ollama+LmStudio) + portable (Export+Import) 4 사이트. emit closure에 cancel 클론 캡처 → send 실패 시 `cancel.cancel()`. ⑦ **R-E.7 (R2, commit `39924ff`)** `WorkspaceCancellationScope` opt-in 등록 인덱스 신설. `set_active_workspace`가 prev_active_id의 op 토큰 cascade cancel. 5 invariant. opt-in 정책 (점진적 register wiring은 v2.x). ⑧ **ADR-0057** (architecture cleanup + protocol decoupling, 7 sub-phase 통합) 신설. 결정 노트 `phase-r-e-architecture-cleanup-decision.md` (6 섹션, 14 기각안). **검증**: cargo workspace clippy -D warnings clean / fmt clean / ACL drift 0 / 회귀 0건. 신규 invariant 29건. **v0.0.1 ship 가능 — `git tag v0.0.1 && git push origin v0.0.1`**. 다음 standby = release tag push 사용자 결정 또는 분리된 sub-phase #31 / #38.
- **2026-05-03 Phase R-D — Frontend Polish 머지** (ADR-0056 채택). GPT Pro 30-issue 중 v0.0.1 ship-blocker i18n / 에러 노출(K1+K2+K3+K4) 4건 일괄 해소. ① **R-D.1 (K1 i18n 이모지 거부)**: `apps/desktop/src/i18n/{ko,en}.json`에서 컬러 이모지 11건(`✅❌⚠ℹ`) 제거 — `diagnostics.signature.{verified,failed,missing,bundled,disabled}` + `catalog.commercial.chip` + tooltip prose 내부 emoji 참조 정리. CLAUDE.md §4.3 lucide-react 정책 일관. ② **Diagnostics SignatureSection lucide wiring**: `signatureIcon(tone)` helper 신규 — tone(ok/warn/error/neutral) → `CheckCircle2 / AlertTriangle / XCircle / Info` 매핑. message `<p>`에 `<Icon size={14} aria-hidden>` + `<span>` 분리. ③ **R-D.2 (K2 errors namespace)**: 최상위 `errors` namespace 신설 + 3 키(`path-denied` / `path-traversal` / `generic`) ko/en 양쪽. R-A의 PathDenied + R-C의 derive_filename InvalidSpec 한국어/영어 토글 친화. ④ **R-D.3 (K3 Catalog hardcoded 정리)**: `Catalog.tsx` 5건 한국어 hardcoded prose → `catalog.refresh.{lastAt, never, title, busy, action}` i18n 키. `catalog.adultContent.toggleTitle` defaultValue stale `⚠ chip` 참조도 갱신. ⑤ **R-D.4 (K4 thiserror frontend 노출 정책)**: `apps/desktop/src/ipc/portable.ts::PortableApiError` union에 `path-denied` variant 추가. `PortableImportPanel.tsx` catch 블록에 kind 기반 분기 — `e.kind === "path-denied"` → `setError("errors.path-denied")` (i18n 키). 기존 raw message fallback 유지(백워드 호환). ⑥ **ADR-0056** (i18n consistency + thiserror frontend exposure) 신설. 결정 노트 `docs/research/phase-r-d-frontend-polish-decision.md` (6 섹션, 14 기각안). RESUME.md + docs/adr/README.md 인덱스 갱신. **검증**: cargo workspace clippy clean / fmt clean / pnpm exec tsc -b clean / vitest sweep 회귀 0건. 다음 standby = **Phase R-E (POST v0.0.1)** — 아키텍처 cleanup 7건은 v0.0.1 ship 후 진입.
- **2026-05-03 Phase R-C — Network + Correctness 머지** (ADR-0055 채택). GPT Pro 30-issue 중 v0.0.1 ship-blocker 신뢰성/네트워크 카테고리(S7+C1+R3+C3) 4건 일괄 해소. ① **R-C.1 (S7+R3)**: 외부 통신 reqwest::Client 7개 사이트(registry-fetcher / auto-updater / installer/downloader / knowledge-stack/embed_download / telemetry / lib.rs HF cron / hf_search)에 `.no_proxy()` 추가 + 9개 사이트의 `.unwrap_or_else(|_| reqwest::Client::new())` 폴백 제거 → `.expect("reqwest Client builder must succeed (TLS init)")`로 통일. 폴백이 정책(.no_proxy + timeout) 우회 통로였음. ② **R-C.2 (C1)**: chat_stream 3개 어댑터(adapter-ollama / adapter-lmstudio / adapter-llama-cpp) 모두 `delta_emitted: bool` flag 추가. transport 에러 발생 시 delta 1건 이상 emit됐으면 `tracing::warn!` + Completed 마감 (graceful early disconnect) — delta 0건이면 기존대로 Failed. EOF(None)는 기존대로 Completed. ③ **R-C.4 (C3)**: `crates/installer/src/action.rs::derive_filename`에 path traversal 검증 5건 추가 — `.` / `..` 단독 거부 + `/`/`\\` (Windows path separator) 거부 + 제어 문자(`\0`, `\n` 등) 거부 + Windows drive letter prefix(`C:`, `D:` ...) 거부. cache_dir.join 시 escape 차단. **6 회귀 invariant** (정상 케이스 + 5 거부 케이스 + path 중간 `..` 통과). ④ **ADR-0055** (Network policy + stream resilience) 신설. 결정 노트 `docs/research/phase-r-c-network-correctness-decision.md` (6 섹션, 14 기각안). RESUME.md + docs/adr/README.md 인덱스 갱신. **검증**: cargo workspace clippy -D warnings clean / fmt clean / installer 7 derive_filename invariant (6 신규) / 회귀 0건. 다음 standby = **Phase R-D.1 (K1+K2+K3 i18n 마무리)**.
...
9. **새 model manifest 추가 시 `build-catalog-bundle.mjs` 필수 실행**: `manifests/snapshot/models/<cat>/<id>.json` 추가만으로는 사용자 카탈로그에 노출 X. *진실의 출처*는 `manifests/apps/catalog.json` 단일 합본 (ADR-0044). 누락 시 .exe 빌드 산출물도 옛 catalog.json 번들 + jsdelivr fetch도 변경 없음 → **사용자가 영영 새 모델 못 봄**. 2026-05-06 nemotron-3-nano-4b 추가 시 본 step 누락 발견 (commit 96d6903 보완). 다음 모델 추가 시 흐름: (1) `manifests/snapshot/models/<cat>/<id>.json` 추가 (2) `node .claude/scripts/build-catalog-bundle.mjs` 실행 (3) `manifests/apps/catalog.json` 차분 확인 (4) commit + push (5) jsdelivr 1~24h propagate (6) 사용자 측 6h cron 또는 수동 새로고침.
