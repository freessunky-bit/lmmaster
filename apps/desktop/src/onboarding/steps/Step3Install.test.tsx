...
describe("Step3Install", () => {
  ...
  it("skip 상태 — 안내 banner 노출 (1.2초 후 자동 done은 머신 책임)", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "onboarding.install.skip.title" }),
    ).toBeInTheDocument();
  });
  ...
  it("running 상태 — 진행률 + phase 라벨 + 그만두기 버튼", async () => {
    ...
    render(<Step3Install />);
    expect(
      screen.getByRole("progressbar", { name: "onboarding.install.progressAria" }),
    ).toBeInTheDocument();
    await user.click(
      screen.getByRole("button", { name: "onboarding.install.cancel" }),
    );
  ...
  it("failed 상태 — 에러 메시지 + RETRY 버튼 (500ms debounce)", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "onboarding.error.retry" }),
    );
