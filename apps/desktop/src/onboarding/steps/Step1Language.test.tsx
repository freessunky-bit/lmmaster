...
describe("Step1Language", () => {
  it("렌더 — 제목과 두 라디오 옵션 노출", () => {
    ...
    render(<Step1Language />);
    expect(
      screen.getByRole("heading", { name: "onboarding.language.title" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("radio", { name: "onboarding.language.option.ko" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("radio", { name: "onboarding.language.option.en" }),
    ).toBeInTheDocument();
  });
