...
const OPENED_URL_OUTCOME = {
    kind: "opened-url",
    url: "https://lmstudio.ai/",
};
// 영원히 pending인 actor — running state 진입 자체만 검사할 때.
const pendingActor = () => fromPromise(() => new Promise(() => { }));
// ── 1. language ────────────────────────────────────────────────────────
describe("머신 초기 상태", () => {
...
describe("scan 서브상태", () => {
    it("scan.running → done with env", async () => {
        ...
        const a = createActor(m).start();
        a.send({ type: "NEXT" });
        await vi.waitFor(() => expect(a.getSnapshot().value).toEqual({ scan: "done" }));
    ...
    it("scan.running → failed → RETRY → idle → running → done", async () => {
        ...
        const a = createActor(m).start();
        a.send({ type: "NEXT" });
        await vi.waitFor(() => expect(a.getSnapshot().value).toEqual({ scan: "failed" }));
...
describe("install 서브상태", () => {
    ...
    const reachInstall = async (env = FAKE_ENV, installActor = fromPromise(() => new Promise(() => { }))) => {
        const m = onboardingMachine.provide({
            actors: {
    ...
    it("running → success outcome → done (Issue A 가드 미적용 분기)", async () => {
        const a = await reachInstall(FAKE_ENV, fromPromise(async () => SUCCESS_OUTCOME));
        a.send({ type: "SELECT_MODEL", id: "ollama" });
    ...
    it("Issue A — running → opened-url outcome → openedUrl substate (자동 done 차단)", async () => {
        const a = await reachInstall(FAKE_ENV, fromPromise(async () => OPENED_URL_OUTCOME));
        a.send({ type: "SELECT_MODEL", id: "lm-studio" });
        await vi.waitFor(() => expect(a.getSnapshot().value).toEqual({ install: "openedUrl" }));
    ...
    it("Issue B — failed → RETRY 시 stale outcome/error 정리 (running.entry: clearInstallState)", async () => {
        ...
        a.send({ type: "NEXT" });
        a.send({ type: "SELECT_MODEL", id: "ollama" });
        await vi.waitFor(() => expect(a.getSnapshot().value).toEqual({ install: "failed" }));
