...
export type OnboardingLang = "ko" | "en";

/** install 진행률 — 매 INSTALL_EVENT의 download.progress가 들어올 때 갱신. */
...
export type OnboardingContext = {
  lang: OnboardingLang;
  modelId?: string;
  /** 환경 점검 결과 — Step 2 성공 시 채워짐. 영속화 대상 아님. */
  env?: EnvironmentReport;
  /** Step 2 점검 실패 메시지. */
  scanError?: string;

  // ── Step 3 install 휘발 상태 ──
  /** 가장 최근 InstallEvent — UI가 phase·구체 렌더링에 사용. */
  installLatest?: InstallEvent;
  /** 마지막 10건 이벤트 로그 — "자세히 보기" 표시용. */
  installLog?: InstallEvent[];
  /** progress 헤더 (downloaded/total/speed_bps). */
  installProgress?: InstallProgressView;
  /** install actor가 onDone에 반환한 ActionOutcome. */
  installOutcome?: ActionOutcome;
  /** 실패 코드/메시지 — failed substate UI에서 매핑. */
  installError?: { code: string; message: string };
  /** Retrying 이벤트의 attempt — UI "(2차 시도)" 노출. */
  retryAttempt?: number;
...
export type OnboardingEvent =
  ...
  | { type: "RETRY" }
  | { type: "RESET_ERROR" }
  | { type: "INSTALL_EVENT"; event: InstallEvent }
  | { type: "RESET_INSTALL" };

/** install actor input — 머신이 invoke할 때 context에서 derive. 테스트에서 mock fromPromise 시 재사용. */
export type InstallActorInput = { id: string };
...
export const onboardingMachine = setup({
  ...
  actors: {
     ...
     */
    install: fromPromise<ActionOutcome, InstallActorInput>(
      async ({ input, signal }) => {
        const onAbort = () => {
        ...
        try {
          return await installApp(input.id, {
            onEvent: (e) => {
              if (signal.aborted) return; // 늦은 이벤트 드롭
  ...
  actions: {
    setLang: assign({
      lang: ({ event }) => (event as Extract<OnboardingEvent, { type: "SET_LANG" }>).lang,
    }),
    setModel: assign({
      modelId: ({ event }) =>
        (event as Extract<OnboardingEvent, { type: "SELECT_MODEL" }>).id,
    }),
    setEnv: assign({
      env: ({ event }) => {
        const out = (event as { output?: EnvironmentReport }).output;
        return out;
      },
      scanError: () => undefined,
    }),
    ...
    applyInstallEvent: assign(({ context, event }) => {
      const ev = (event as Extract<OnboardingEvent, { type: "INSTALL_EVENT" }>).event;
    ...
    setOutcome: assign({
      installOutcome: ({ event }) => {
        const out = (event as { output?: ActionOutcome }).output;
        return out;
      },
  ...
  guards: {
    ...
    anyRuntimeRunning: ({ context }) => {
      ...
      return rs.some(
        (r) =>
          (r.runtime === "ollama" || r.runtime === "lm-studio") &&
          r.status === "running",
      );
    },
    /**
     ...
     */
    isOpenedUrlOutcome: ({ event }) =>
      (event as { output?: ActionOutcome }).output?.kind === "opened-url",
  },
}).createMachine({
  id: "onboarding",
  initial: "language",
...
export type OnboardingActor = ReturnType<typeof onboardingMachine.provide>;
...
export function sanitizeSnapshotForPersist(snapshot: {
  ...
  if (
    ...
    typeof value === "object" &&
    "install" in value &&
    typeof (value as { install: unknown }).install === "string"
  ) {
  ...
  if (
    ...
    typeof value === "object" &&
    "scan" in value &&
    typeof (value as { scan: unknown }).scan === "string"
  ) {
    value = "scan";
  }
