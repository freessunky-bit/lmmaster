...
import { CommandPaletteProvider, useCommandPalette, useCommandRegistration, } from "../components/command-palette/context";
...
const FAKE_ENV = {
    ...
    runtimes: [
        { runtime: "ollama", status: "not-installed" },
...
describe("접근성 — 마법사 step 컴포넌트", () => {
    it("Step1Language WCAG 위반 없음", async () => {
        const { container } = render(_jsx(Step1Language, {}));
    ...
    it("Step2Scan running 상태 WCAG 위반 없음", async () => {
        ...
        const { container } = render(_jsx(Step2Scan, {}));
        const results = await axe(container, AXE_OPTIONS);
        expect(results.violations).toEqual([]);
    });
    ...
    it("Step3Install idle 상태 WCAG 위반 없음", async () => {
        ...
        vi.mocked(ctx.useOnboardingEnv).mockReturnValue(FAKE_ENV);
        const { container } = render(_jsx(Step3Install, {}));
        const results = await axe(container, AXE_OPTIONS);
        expect(results.violations).toEqual([]);
    });
    ...
    it("Step4Done WCAG 위반 없음", async () => {
        const { container } = render(_jsx(Step4Done, { onFinish: vi.fn() }));
        const results = await axe(container, AXE_OPTIONS);
        expect(results.violations).toEqual([]);
    });
});
...
describe("접근성 — CommandPalette", () => {
    it("열린 상태 WCAG 위반 없음", async () => {
        ...
        const { container } = render(_jsxs(CommandPaletteProvider, { children: [_jsx(OpenController, { open: true }), _jsx(CommandsRegister, { commands: cmds }), _jsx(CommandPalette, {})] }));
