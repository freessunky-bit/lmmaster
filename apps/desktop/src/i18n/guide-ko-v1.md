...
## 가상 한국인 설문 시뮬레이션 (Personas-Korea)

엔비디아가 공개한 *Personas-Korea* 데이터셋(700만 합성 한국인 페르소나) + **NVIDIA Nemotron 3 Nano 4B** 또는 **EXAONE 3.5 7.8B**로 가상 한국인 100인의 콘텐츠 설문 응답을 만들 수 있어요.

- **카탈로그 → 🔥 NEW** 탭에서 Nemotron 3 Nano 4B를 설치해요. 4B + Mamba-2 하이브리드라 8GB RAM PC에서도 빠르게 돌아요.
- 데이터셋은 NVIDIA의 [Personas-Korea](https://huggingface.co/datasets/nvidia/Nemotron-Personas-Korea)에서 직접 받아요 (CC BY 4.0, 상업 사용 가능).
- step-by-step은 프로젝트 `docs/guides/personas-korea-survey-simulation.md` 참고. 콘텐츠 A/B 테스트, 마케팅 카피 사전 검증, UX 페르소나 리서치에 잘 맞아요.
- 한국어 *자연스러움*이 더 중요하면 EXAONE 3.5 7.8B로 교체하세요. Nemotron은 다국어 모델이라 한국어 단독 자연스러움은 한 단계 낮아요.
...
엔비디아 *Nemotron-Personas-Korea* 데이터셋(700만 합성 한국인 페르소나)과 **로컬 LLM**으로 가상 한국인 100명에게 콘텐츠 설문을 시뮬레이션해요. 콘텐츠 A/B 테스트, 마케팅 카피 사전 검증, UX 페르소나 리서치에 써요.
...
1. **모델 설치** — 카탈로그 → 🔥 NEW → **NVIDIA Nemotron 3 Nano 4B** (또는 한국어 자연스러움 우선이면 **EXAONE 3.5 7.8B**).
2. **데이터셋 다운로드** — Python으로 `nvidia/Nemotron-Personas-Korea` (~1.8GB Parquet, CC BY 4.0).
3. **100인 stratified 샘플링** — 성별·연령·지역 비례. `random_state=42`로 재현성.
4. **설문지 정의** — JSON으로 객관식 / 척도 / 주관식 정의.
5. **Workbench batch 실행** — LMmaster Local API (`http://127.0.0.1:8788/v1`)에 100인 × N문항 호출.
6. **결과 집계** — pandas crosstab + 키워드 빈도.

각 단계 코드는 프로젝트 `docs/guides/personas-korea-survey-simulation.md`에 Python 스크립트 4개 (`download_personas.py` / `sample_100.py` / `run_survey.py` / `aggregate.py`) 완비예요.

## 모델 선택 가이드

- **빠르고 4090 배치 처리** → Nemotron 3 Nano 4B (Mamba-2 + Transformer, 4B Dense, 한국어 sprinkle).
- **한국어 자연스러움 우선** → EXAONE 3.5 7.8B 또는 HCX-SEED 8B (한국어 native 학습).
- **고사양 RTX 4090 + 정성 ↑** → Nemotron 3 Nano 30B A3B (MoE 활성 3.5B, Q3_K_M 20GB).

## 자주 만나는 문제

- 한국어 답변에 영어 단어 섞임 → system prompt에 "*반드시 한국어로만*" 강조 + EXAONE으로 교체.
- 모든 페르소나가 비슷한 답 → `persona` + `cultural_background` 둘 다 system prompt에. temperature 0.8~0.9.
- 객관식이 보기 외 답변 → user prompt 끝에 "답변(보기 중 하나):" 추가.
- 응답이 너무 짧음 → `max_tokens` 256 → 512.

## 라이선스 표기

연구·블로그 게시 시:
- **데이터**: NVIDIA Nemotron-Personas-Korea (KOSIS·대법원·건강보험공단·한국농촌경제연구원·NAVER Cloud 5개 출처 표기 필수, CC BY 4.0).
- **모델**: NVIDIA Nemotron 3 Nano 4B (NVIDIA Open Model License, 상업 OK).

## 다가올 1-click 자동화

Phase 23'.c.2 (다음 sub-phase)에서 *"내 워크스페이스에 추가" 버튼 1번*으로:
- 자동 다운로드 + chunk + 임베딩 + SQLCipher 저장
- Workspace > 지식 자료 탭에 자동 등장
- 채팅에서 RAG 컨텍스트 토글로 즉시 사용

지금은 위 6단계 수동이지만, 1-click이 도착하면 *Python 셋업 0*으로 가능해져요.
