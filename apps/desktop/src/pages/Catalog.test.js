...
describe("CatalogPage 렌더", () => {
    ...
    it("카테고리 변경 시 그리드 필터링", async () => {
        ...
        const { container } = render(_jsx(CatalogPage, {}));
        await waitFor(() => {
            expect(screen.getAllByText("EXAONE 4.0 1.2B").length).toBeGreaterThan(0);
        });
        ...
        await waitFor(() => {
            ...
            const titles = within(grid).getAllByRole("listitem");
            expect(titles.length).toBe(1);
            expect(within(titles[0]).getByText("Qwen 2.5 Coder")).toBeTruthy();
        });
    });
    ...
    it('"추천만" 토글하면 그리드가 추천 4슬롯 모델만 남김', async () => {
        ...
        await waitFor(() => {
            ...
            const titles = within(grid).getAllByRole("listitem");
            expect(titles.length).toBe(1);
            expect(within(titles[0]).getByText("EXAONE 4.0 1.2B")).toBeTruthy();
        });
    });
    ...
    it("'설치 크기 작은순'으로 정렬 변경", async () => {
        ...
        await waitFor(() => {
            const titles = screen
                ...
                .map((el) => within(el).queryByRole("button")?.textContent ?? "")
