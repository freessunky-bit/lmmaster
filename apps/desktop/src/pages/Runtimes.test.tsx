...
vi.mock("@lmmaster/design-system/react", () => ({
  ...
  VirtualList: <T,>(props: any) => (
    <div role="list" aria-label={props.ariaLabel}>
      {(props.items as T[]).map((item, idx) => (
        <div
          key={props.keyOf(item)}
          role="listitem"
          className="ds-vlist-row"
          style={{ height: props.rowHeight ?? 24 }}
        >
          {props.renderRow(item, idx)}
        </div>
      ))}
    </div>
  ),
...
const FIXTURE_STATUSES: RuntimeStatus[] = [
  {
    kind: "ollama",
    installed: true,
    version: "0.4.0",
...
const FIXTURE_OLLAMA_MODELS: RuntimeModelView[] = [
  {
    runtime_kind: "ollama",
    id: "exaone:1.2b",
...
const FIXTURE_LMSTUDIO_EMPTY: RuntimeModelView[] = [];
...
beforeEach(() => {
  ...
  vi.mocked(listRuntimeModels).mockImplementation(async (kind) => {
    if (kind === "ollama") return FIXTURE_OLLAMA_MODELS;
...
describe("RuntimesPage 렌더", () => {
  ...
  it("LM Studio 카드 클릭 → 모델 0개 → 빈 상태 + CTA 표시", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("screens.runtimes.models.empty.title"),
      ).toBeTruthy();
      expect(
        screen.getByText("screens.runtimes.models.empty.cta"),
      ).toBeTruthy();
    });
  });
  ...
  it("빈 상태 CTA 클릭 → catalog navigate event 발생", async () => {
    ...
    const detail = (navEvents[0] as CustomEvent).detail;
  ...
  it("start/stop/restart/logs 버튼 모두 disabled", async () => {
    ...
    const stopBtn = within(ollamaCard).getByText(
      "screens.runtimes.card.actions.stop",
    );
    const restartBtn = within(ollamaCard).getByText(
      "screens.runtimes.card.actions.restart",
    );
    const logsBtn = within(ollamaCard).getByText(
      "screens.runtimes.card.actions.logs",
    );
