...
vi.mock("../ipc/knowledge", () => {
    return {
        isTerminalIngestEvent: (ev) => ev.kind === "done" || ev.kind === "failed" || ev.kind === "cancelled",
        startIngest: vi.fn(),
        cancelIngest: vi.fn(),
...
describe("Workspace Phase 4.5'.b — Knowledge tab", () => {
    it("Knowledge 탭 진입 — stats 로드 + 0/0 표기", async () => {
        ...
        render(_jsx(Workspace, { workspaceId: "default" }));
        // stats가 비동기로 로드됨.
        await waitFor(() => {
    ...
    it("path 비어있을 때 인덱싱 시작 버튼 disabled", () => {
        render(_jsx(Workspace, { workspaceId: "default" }));
        const start = screen.getByTestId("workspace-ingest-start");
    ...
    it("path 입력 후 인덱싱 시작 → startIngest 호출 (config 전달)", async () => {
        ...
        render(_jsx(Workspace, { workspaceId: "ws-1", storePath: "/tmp/store.db" }));
    ...
    it("진행 이벤트 — Chunking event 도착 시 progressbar 갱신 + 입력 disabled", async () => {
        ...
        const user = userEvent.setup();
        render(_jsx(Workspace, { workspaceId: "ws", storePath: "" }));
        await waitFor(() => expect(statsMock).toHaveBeenCalled());
        const pathInput = screen.getByTestId("workspace-ingest-path");
        ...
        // 입력은 disabled.
        expect(screen.getByTestId("workspace-ingest-path").disabled).toBe(true);
        // Cancel 버튼 노출.
    ...
    it("검색 — query 입력 + submit → searchKnowledge 호출 + hits 노출", async () => {
        ...
        const user = userEvent.setup();
        render(_jsx(Workspace, { workspaceId: "ws-search", storePath: "/tmp/k.db" }));
        await waitFor(() => expect(statsMock).toHaveBeenCalled());
    ...
    it("a11y violations 없음 (기본 idle 상태)", async () => {
        const { container } = render(_jsx(Workspace, { workspaceId: "default" }));
        await waitFor(() => expect(statsMock).toHaveBeenCalled());
