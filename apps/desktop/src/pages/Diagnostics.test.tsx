...
import { getGatewayStatus, type GatewayState } from "../ipc/gateway";
import {
...
const FAKE_GW: GatewayState = {
  port: 7373,
  status: "listening",
  error: null,
};

const FAKE_WS: WorkspaceStatus = {
  fingerprint: {
    os: "windows",
    arch: "x86_64",
    gpu_class: "nvidia",
...
describe("Diagnostics 4-grid 렌더", () => {
  ...
  it("scanner mock — getLastScan resolves with summary → 좌상에 텍스트 표시", async () => {
    render(<Diagnostics />);
    await waitFor(() => {
      const scanSection = screen.getByTestId("diag-section-scan") as HTMLElement;
      expect(
        within(scanSection).getByText(FAKE_SCAN.summary_korean),
      ).toBeTruthy();
    });
  });
  ...
  it("scanner null → 좌상에 빈 상태 메시지 표시", async () => {
    ...
    await waitFor(() => {
      const scanSection = screen.getByTestId("diag-section-scan") as HTMLElement;
      expect(
        within(scanSection).getByText("screens.diagnostics.sections.scan.empty"),
      ).toBeTruthy();
    });
  });
  ...
  it("gateway StatusPill 표시 (status + port detail)", async () => {
    render(<Diagnostics />);
    await waitFor(() => {
      const gwSection = screen.getByTestId("diag-section-gateway") as HTMLElement;
  ...
  it("workspace fingerprint mock → 우하 섹션에 os/arch 표시", async () => {
    render(<Diagnostics />);
    await waitFor(() => {
      const wsSection = screen.getByTestId("diag-section-workspace") as HTMLElement;
  ...
  it('"새 측정 시작" 버튼 클릭 → navigation custom event 발생', async () => {
    ...
    const evt = onNav.mock.calls[0]![0] as CustomEvent;
