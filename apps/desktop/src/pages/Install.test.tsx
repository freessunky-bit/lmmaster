...
vi.mock("../ipc/environment", async () => {
  const actual = await vi.importActual<typeof import("../ipc/environment")>(
    "../ipc/environment",
  );
  return {
    ...actual,
...
const FAKE_HW: EnvironmentReport["hardware"] = {
...
function makeEnv(
  ...
  lmStudio: "running" | "installed" | "not-installed",
): EnvironmentReport {
  return {
    hardware: FAKE_HW,
    runtimes: [
...
beforeEach(() => {
  vi.mocked(detectEnvironment).mockResolvedValue(
    makeEnv("not-installed", "not-installed"),
  );
...
describe("InstallPage 렌더", () => {
  it("두 카드 (Ollama + LM Studio) 렌더 + alias / license / reason 표시", async () => {
    ...
    expect(
      within(ollamaCard).getByText("screens.install.cards.ollama.name"),
    ).toBeTruthy();
    expect(
      within(ollamaCard).getByText("screens.install.cards.ollama.license"),
    ).toBeTruthy();
    expect(
      within(ollamaCard).getByText("screens.install.cards.ollama.reason"),
    ).toBeTruthy();
    // LM Studio.
    expect(
      within(lmCard).getByText("screens.install.cards.lmStudio.name"),
    ).toBeTruthy();
    expect(
      within(lmCard).getByText("screens.install.cards.lmStudio.license"),
    ).toBeTruthy();
    expect(
      within(lmCard).getByText("screens.install.cards.lmStudio.reason"),
    ).toBeTruthy();
  });
...
describe("InstallPage drawer", () => {
  it("카드 details 클릭 → drawer 열림 (role='dialog') + Esc 닫힘", async () => {
    ...
    const cards = container.querySelectorAll(".install-card");
    const detailsBtn = within(cards[0]! as HTMLElement).getByRole("button", {
  ...
  it("drawer에 license / install size / homepage 노출", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        within(dialog).getByText("screens.install.drawer.licenseFull"),
      ).toBeTruthy();
      expect(
        within(dialog).getByText("screens.install.drawer.installSize"),
      ).toBeTruthy();
      expect(
        within(dialog).getByText("screens.install.drawer.homepage"),
      ).toBeTruthy();
    });
  });
});
...
describe("InstallPage 빈 상태", () => {
  it("두 런타임 모두 running일 때 빈 상태 + CTA", async () => {
    vi.mocked(detectEnvironment).mockResolvedValue(
      makeEnv("running", "running"),
    );
    const onNavigate = vi.fn();
    render(<InstallPage onNavigate={onNavigate} />);
    await waitFor(() => {
  ...
  it("한 런타임만 installed면 빈 상태 안 나옴", async () => {
    vi.mocked(detectEnvironment).mockResolvedValue(
      makeEnv("installed", "not-installed"),
    );
    render(<InstallPage />);
    await waitFor(() => {
...
describe("InstallPage 진행 패널", () => {
  it("설치 시작 시 InstallProgress compact + 취소 버튼 노출", async () => {
    ...
    const cards = container.querySelectorAll(".install-card");
    const firstCardInstall = within(cards[0]! as HTMLElement).getAllByRole(
      "button",
      { name: /screens\.install\.actions\.install/ },
    );
    ...
    await waitFor(() => {
      ...
      const cancelBtn = within(panel as HTMLElement).getByRole("button", {
