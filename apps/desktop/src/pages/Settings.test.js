...
describe("Settings 페이지", () => {
    it("4 카테고리 nav 모두 렌더 + 일반이 default", async () => {
        ...
        // 일반이 active.
        expect(screen.getByTestId("settings-category-general").getAttribute("aria-checked")).toBe("true");
    ...
    it("일반 → 자가스캔 주기 라디오 → setScanInterval + localStorage 갱신", async () => {
        ...
        expect(radio15.checked).toBe(false);
        await user.click(radio15);
        await waitFor(() => {
            expect(radio15.checked).toBe(true);
        });
        expect(globalThis.localStorage.getItem("lmmaster.settings.general.scan_interval_min")).toBe("15");
        const radioOff = screen.getByLabelText("screens.settings.general.scanInterval.off");
        await user.click(radioOff);
        await waitFor(() => {
            expect(radioOff.checked).toBe(true);
        });
        expect(globalThis.localStorage.getItem("lmmaster.settings.general.scan_interval_min")).toBe("0");
    });
    ...
    it("일반 → 테마 light 라디오는 disabled (lock 아이콘 + selectable 불가)", async () => {
        render(_jsx(Settings, {}));
        const light = screen.getByLabelText(/screens\.settings\.general\.theme\.light/);
        expect(light.disabled).toBe(true);
        const dark = screen.getByLabelText("screens.settings.general.theme.dark");
    ...
    it("워크스페이스 → '지금 정리할게요' 클릭 → checkWorkspaceRepair 호출", async () => {
        ...
        // 결과 메시지 표시.
        await waitFor(() => {
            expect(screen.getByText(/screens\.settings\.workspace\.repairDone/)).toBeTruthy();
        });
    });
    it("워크스페이스 → 경로 표시 + 'relocate'는 disabled", async () => {
        ...
        await user.click(screen.getByTestId("settings-category-workspace"));
        await waitFor(() => {
            expect(screen.getByText("C:\\Users\\me\\AppData\\Local\\LMmaster")).toBeTruthy();
        });
        const relocateBtn = screen.getByRole("button", {
            name: /screens\.settings\.workspace\.relocate$/,
        });
        expect(relocateBtn.disabled).toBe(true);
    });
    it("카탈로그 → registryUrl read-only", async () => {
        ...
        const input = screen.getByTestId("settings-registry-url");
    ...
    it("고급 → Gemini / 로그 export 모두 disabled + comingSoon 텍스트", async () => {
        ...
        expect(geminiToggle.disabled).toBe(true);
        expect(geminiToggle.getAttribute("aria-checked")).toBe("false");
        // comingSoon 텍스트.
        expect(screen.getByText("screens.settings.advanced.gemini.comingSoon")).toBeTruthy();
    ...
    it("고급 → SQLCipher env hint 표시 (LMMASTER_ENCRYPT_DB)", async () => {
        ...
        await user.click(screen.getByTestId("settings-category-advanced"));
        await waitFor(() => {
            expect(screen.getByText("screens.settings.advanced.sqlcipher")).toBeTruthy();
        });
        expect(screen.getByText(/LMMASTER_ENCRYPT_DB=/)).toBeTruthy();
        expect(screen.getByText("screens.settings.advanced.sqlcipher.envHint")).toBeTruthy();
    });
    it("a11y — 위반 0건 (axe)", async () => {
        const user = userEvent.setup();
        const { container } = render(_jsx(Settings, {}));
...
describe("Settings — initial localStorage hydration", () => {
    it("자가스캔 주기 — localStorage에 저장된 값이 있으면 그것을 active로", async () => {
        globalThis.localStorage.setItem("lmmaster.settings.general.scan_interval_min", "15");
        render(_jsx(Settings, {}));
        await waitFor(() => {
            const radio15 = screen.getByLabelText("screens.settings.general.scanInterval.15m");
            expect(radio15.checked).toBe(true);
...
describe("Settings — workspace repair 실패", () => {
    it("checkWorkspaceRepair reject 시 에러 메시지", async () => {
        ...
        await user.click(screen.getByTestId("settings-workspace-repair-btn"));
        await waitFor(() => {
            expect(screen.getByText("screens.settings.workspace.errorRepair")).toBeTruthy();
        });
        warnSpy.mockRestore();
    });
});
describe("Settings — 자동 갱신 (Phase 6'.b)", () => {
    ...
    it("토글 ON → startAutoUpdatePoller 호출 (6h 기본)", async () => {
        ...
        await waitFor(() => {
            expect(startAutoUpdatePoller).toHaveBeenCalledWith("anthropics/lmmaster", expect.any(String), 6 * 3600, expect.any(Function));
        });
    });
    it("active 상태에서 토글 OFF → stopAutoUpdatePoller 호출", async () => {
        ...
        render(_jsx(Settings, {}));
        await waitFor(() => {
            expect(screen.getByText("screens.settings.autoUpdate.toggleOn")).toBeTruthy();
        });
        // 토글 OFF.
    ...
    it("interval radio 토글 비활성 상태에선 disabled", async () => {
        ...
        const r1h = screen.getByLabelText("screens.settings.autoUpdate.interval.1h");
    ...
    it("interval radio active 상태에선 클릭 → restart (stop + start)", async () => {
        ...
        const r12h = screen.getByLabelText("screens.settings.autoUpdate.interval.12h");
        ...
        await waitFor(() => {
            expect(startAutoUpdatePoller).toHaveBeenLastCalledWith("anthropics/lmmaster", expect.any(String), 12 * 3600, expect.any(Function));
        });
    });
    ...
    it("UpToDate event → '최신 버전이에요' info 메시지", async () => {
        ...
        await user.click(screen.getByTestId("settings-autoupdate-check-now"));
        await waitFor(() => {
            expect(screen.getByText("screens.settings.autoUpdate.upToDate")).toBeTruthy();
        });
    });
    it("Failed event → 한국어 에러 메시지", async () => {
        ...
        await waitFor(() => {
            ...
            expect(screen.getByText(/screens\.settings\.autoUpdate\.errorCheck.*네트워크/)).toBeTruthy();
        });
    });
    it("status에서 last_check_iso 있으면 lastChecked 메시지", async () => {
        ...
        render(_jsx(Settings, {}));
        await waitFor(() => {
            expect(screen.getByText(/screens\.settings\.autoUpdate\.lastChecked.*2026-04-28T01:23:45Z/)).toBeTruthy();
        });
    });
    it("startAutoUpdatePoller reject 시 에러 메시지 + 토글 비활성 유지", async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText("screens.settings.autoUpdate.errorStart")).toBeTruthy();
        });
        warnSpy.mockRestore();
    });
