...
vi.mock("../ipc/workbench", () => {
  return {
    isTerminalEvent: (ev: { kind: string }) =>
      ev.kind === "completed" || ev.kind === "failed" || ev.kind === "cancelled",
...
describe("Workbench Phase 5'.b — 5단계 작업대", () => {
  it("Step 1 (data) 진입 — 기본 base_model 입력 + 시작 버튼", () => {
    render(<Workbench />);
    const baseInput = screen.getByTestId("wb-input-base-model") as HTMLInputElement;
  ...
  it("시작 버튼 클릭 → startWorkbenchRun 호출 (config 전달)", async () => {
    ...
    startMock.mockImplementation(async (_config, opts) => {
      capturedOnEvent = opts.onEvent as (ev: { kind: string }) => void;
      return {
        run_id: "uuid-test",
  ...
  it("running 상태 — Started/StageProgress event 도착 시 UI 갱신", async () => {
    ...
    startMock.mockImplementation(async (config, opts) => {
      onEvent = opts.onEvent as (ev: any) => void;
      return handle;
    });
    const user = userEvent.setup();
    render(<Workbench />);
  ...
  it("Quantize step — 4가지 quant type radiogroup + 선택", async () => {
    ...
    // 기본 Q4_K_M 선택됨.
    expect(screen.getByTestId("wb-quant-Q4_K_M")).toHaveAttribute(
      "aria-checked",
      "true",
    );
    // Q5_K_M 클릭 → checked 전환.
    await user.click(screen.getByTestId("wb-quant-Q5_K_M"));
    expect(screen.getByTestId("wb-quant-Q5_K_M")).toHaveAttribute(
      "aria-checked",
      "true",
    );
    expect(screen.getByTestId("wb-quant-Q4_K_M")).toHaveAttribute(
  ...
  it("Runtime selector — 3개 옵션 (mock/ollama/lm-studio)", () => {
    ...
    // 기본은 mock 선택.
    expect(screen.getByTestId("wb-runtime-mock")).toHaveAttribute(
      "aria-checked",
      "true",
    );
  });
  ...
  it("Runtime selector — Ollama 선택 시 base URL/model id 입력 노출", async () => {
    ...
    expect(screen.getByTestId("wb-runtime-ollama")).toHaveAttribute(
      "aria-checked",
      "true",
    );
  ...
  it("Runtime selector — LM Studio 선택 시 base URL 노출 + ollama-create 토글 숨김", async () => {
    ...
    expect(screen.getByTestId("wb-runtime-lm-studio")).toHaveAttribute(
      "aria-checked",
      "true",
    );
    expect(screen.getByTestId("wb-runtime-http-fields")).toBeInTheDocument();
  ...
  it("ollama-create-progress 이벤트 — 라인이 누적되어 노출", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("wb-ollama-output-name")).toHaveTextContent(
        "lmmaster-qwen-12345678",
      );
      expect(screen.getByTestId("wb-ollama-create-log")).toHaveTextContent(
