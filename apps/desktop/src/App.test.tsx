...
describe("App shell — workspace nav i18n + 키", () => {
  it("ko/en 모두 nav.workspace 키 노출", () => {
    expect((koLocale as unknown as NavLike).nav.workspace).toBe("워크스페이스");
    expect((enLocale as unknown as NavLike).nav.workspace).toBe("Workspace");
  });

  it("nav 키 셋이 동일 (ko/en symmetry)", () => {
    const koKeys = Object.keys((koLocale as unknown as NavLike).nav).sort();
    const enKeys = Object.keys((enLocale as unknown as NavLike).nav).sort();
    expect(koKeys).toEqual(enKeys);
  });

  it("workspace는 runtimes와 projects 사이에 등장 (sidebar ordering)", () => {
    const koKeys = Object.keys((koLocale as unknown as NavLike).nav);
