...
import { createWorkspace, deleteWorkspace, getActiveWorkspace, listWorkspaces, onWorkspacesChanged, renameWorkspace, setActiveWorkspace, } from "../ipc/workspaces";
...
export function ActiveWorkspaceProvider({ children }) {
    ...
    const setActiveAction = useCallback(async (id) => {
    ...
    const createAction = useCallback(async (name, description) => {
        const info = await createWorkspace(name, description);
        // 이벤트가 list 갱신을 가져오지만 optimistic으로 list에 즉시 추가.
        setWorkspaces((prev) => {
            if (prev.some((w) => w.id === info.id))
                return prev;
            return [...prev, info];
        });
        return info;
    }, []);
    const renameAction = useCallback(async (id, newName) => {
...
export function useActiveWorkspace() {
    ...
    if (!ctx) {
        throw new Error("useActiveWorkspace는 <ActiveWorkspaceProvider> 안에서만 호출할 수 있어요.");
    }
    return ctx;
}
/**
