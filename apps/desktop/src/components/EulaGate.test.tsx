...
function simulateScrollEnd() {
  const body = screen.getByTestId("eula-body") as HTMLDivElement;
...
describe("EulaGate — 첫 렌더 + 동의 흐름", () => {
  it("동의 안 한 상태 — dialog 노출 + 한국어 본문 + 동의 버튼 disabled", async () => {
    render(
      <EulaGate eulaVersion={VERSION} onAccept={vi.fn()}>
        <div data-testid="protected-child">main app</div>
      </EulaGate>,
    );
    ...
    // 한국어 본문 default.
    await waitFor(() => {
      expect(screen.getByTestId("eula-body").textContent ?? "").toContain(
        "KOREAN_EULA_MARKER",
      );
    });
    const accept = screen.getByTestId("eula-accept") as HTMLButtonElement;
  ...
  it("스크롤 끝 도달 → 동의 버튼 활성화", async () => {
    ...
    await waitFor(() => {
      expect(
        (screen.getByTestId("eula-accept") as HTMLButtonElement).disabled,
      ).toBe(false);
    });
  });
  ...
  it("동의 클릭 → localStorage 저장 + onAccept 호출 + 자식 노출", async () => {
    ...
    render(
      <EulaGate eulaVersion={VERSION} onAccept={onAccept}>
        <div data-testid="protected-child">main app</div>
      </EulaGate>,
    );
    simulateScrollEnd();
