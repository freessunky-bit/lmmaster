...
vi.mock("../../ipc/portable", () => ({
    ...
    isTerminalExportEvent: (ev) => ev.kind === "done" || ev.kind === "failed",
}));
import { cancelWorkspaceExport, startWorkspaceExport, } from "../../ipc/portable";
...
describe("PortableExportPanel — 기본 렌더", () => {
    it("idle 상태에서 시작 버튼이 활성화돼요", () => {
        render(_jsx(PortableExportPanel, {}));
        const btn = screen.getByTestId("portable-export-start-btn");
        expect(btn.getAttribute("disabled")).toBeNull();
        expect(screen.getByText("screens.settings.portable.export.subtitle")).toBeTruthy();
    });
...
describe("PortableExportPanel — 검증", () => {
    it("target 미입력 시 에러 키가 노출돼요", async () => {
        ...
        expect(screen.getByText("screens.settings.portable.export.errors.emptyTarget")).toBeTruthy();
    ...
    it("키 포함 + 패스프레이즈 빈 상태로 confirm → 에러", async () => {
        ...
        render(_jsx(PortableExportPanel, {}));
        await user.click(screen.getByTestId("portable-export-start-btn"));
        await user.type(screen.getByTestId("portable-export-target-input"), "C:/tmp/out.zip");
        await user.click(screen.getByTestId("portable-export-include-keys"));
        await user.click(screen.getByTestId("portable-export-confirm-btn"));
        expect(screen.getByText("screens.settings.portable.export.errors.emptyPassphrase")).toBeTruthy();
        expect(startWorkspaceExport).not.toHaveBeenCalled();
    });
});
describe("PortableExportPanel — 진행", () => {
    it("정상 입력 → startWorkspaceExport 호출 + done 카드 노출", async () => {
        ...
        expect(within(done).getByText("screens.settings.portable.export.done")).toBeTruthy();
    });
    it("진행 중 cancel 버튼이 노출되고 클릭 가능해요", async () => {
        let onEventRef = null;
        vi.mocked(startWorkspaceExport).mockImplementationOnce(async (_opts, onEvent) => {
            onEventRef = onEvent;
...
describe("PortableExportPanel — a11y", () => {
    it("axe violations === [] (idle)", async () => {
        const { container } = render(_jsx(PortableExportPanel, {}));
