...
vi.mock("../../ipc/portable", () => ({
  ...
  isTerminalImportEvent: (ev: { kind: string }) =>
...
const PREVIEW_META_ONLY: ArchivePreview = {
...
const PREVIEW_WITH_KEYS: ArchivePreview = {
  ...PREVIEW_META_ONLY,
  has_keys: true,
};
...
describe("PortableImportPanel — verify + preview", () => {
  it("source 미입력 시 검증 시도하면 에러 키 노출", async () => {
    ...
    expect(
      screen.getByText(
        "screens.settings.portable.import.errors.emptySource",
      ),
    ).toBeTruthy();
  ...
  it("정상 경로 입력 → verify → preview 노출", async () => {
    ...
    await user.click(screen.getByTestId("portable-import-start-btn"));
    await user.type(
      screen.getByTestId("portable-import-source-input"),
      "C:/tmp/in.zip",
    );
    await user.click(screen.getByTestId("portable-import-verify-btn"));
    await waitFor(() => {
...
describe("PortableImportPanel — 정책 + 진행", () => {
  it("conflict_policy 라디오 3개 모두 렌더돼요", async () => {
    ...
    expect((renameRadio as HTMLInputElement).checked).toBe(true);
  });
  ...
  it("키 포함 + 패스프레이즈 빈 상태로 confirm → 에러", async () => {
    ...
    expect(
      screen.getByText(
        "screens.settings.portable.import.errors.emptyPassphrase",
      ),
    ).toBeTruthy();
...
describe("PortableImportPanel — a11y", () => {
  it("axe violations === [] (idle)", async () => {
    const { container } = render(<PortableImportPanel />);
