...
import type { BenchReport } from "../../ipc/bench";
...
describe("BenchChip", () => {
  it("idle 상태는 측정하기 CTA 버튼", async () => {
    ...
    render(<BenchChip state={{ kind: "idle" }} onMeasure={onMeasure} />);
  ...
  it("running 상태는 spinner + 취소 CTA", async () => {
    ...
    render(<BenchChip state={{ kind: "running" }} onCancel={onCancel} />);
  ...
  it("측정 완료 ok 상태 — 한 줄 요약 텍스트", () => {
    render(<BenchChip state={{ kind: "report", report: makeReport() }} />);
