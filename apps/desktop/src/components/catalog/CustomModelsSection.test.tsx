...
vi.mock("../../ipc/workbench", async () => {
  const actual = await vi.importActual<typeof import("../../ipc/workbench")>(
    "../../ipc/workbench",
  );
  return {
    ...actual,
...
import { listCustomModels, type CustomModel } from "../../ipc/workbench";
import { CustomModelsSection } from "./CustomModelsSection";

const FIXTURE_MODELS: CustomModel[] = [
  {
    id: "my-coder-q4",
...
describe("CustomModelsSection (Phase 8'.b.1)", () => {
  it("로딩 상태 → 모델 목록 렌더 + badge 표시", async () => {
    ...
    expect(
      within(first).getByText(/screens\.catalog\.custom\.basedOn.*qwen-coder-7b/),
    ).toBeTruthy();
  });
  ...
  it("eval_total=0인 모델은 evalSummary 미노출", async () => {
    ...
    expect(
      screen.queryByText(/screens\.catalog\.custom\.evalSummary/),
    ).toBeNull();
  });
  ...
  it("카드 클릭 → onSelect 콜백이 CustomModel 인자로 호출", async () => {
    ...
    const user = userEvent.setup();
    render(<CustomModelsSection onSelect={onSelect} />);
  ...
  it("scoped 쿼리 — section 안에서 grid 카드만 카운트 (Korean badge가 다른 곳과 충돌 X)", async () => {
    vi.mocked(listCustomModels).mockResolvedValue(FIXTURE_MODELS);
    const { container } = render(<CustomModelsSection />);
    ...
    const cards = within(section as HTMLElement).getAllByTestId(
      "custom-model-card",
    );
    expect(cards.length).toBe(2);
  });
