...
import { isFormControlActive, ShortcutsModal, useShortcutsHotkey, } from "./ShortcutsModal";
beforeEach(() => {
...
describe("ShortcutsModal — 기본 렌더", () => {
    it("open=false면 null 렌더", () => {
        const { container } = render(_jsx(ShortcutsModal, { open: false, onClose: vi.fn() }));
        expect(container.firstChild).toBeNull();
    });
    it("open=true → role=dialog + aria-modal=true + 13행 표시", () => {
        render(_jsx(ShortcutsModal, { open: true, onClose: vi.fn() }));
    ...
    it("close 버튼 → onClose 호출", async () => {
        ...
        const onClose = vi.fn();
        render(_jsx(ShortcutsModal, { open: true, onClose: onClose }));
    ...
    it("a11y violations === []", async () => {
        const { container } = render(_jsx(ShortcutsModal, { open: true, onClose: vi.fn() }));
...
describe("useShortcutsHotkey — F1/Ctrl+숫자", () => {
    function HookHarness({ onNav, initialOpen = false, }) {
        const [open, setOpen] = useState(initialOpen);
        useShortcutsHotkey({ open, setOpen, onNav });
        return _jsx("div", { "data-testid": "hook-state", children: open ? "open" : "closed" });
    }
    it("F1 → state 토글 (closed → open)", async () => {
        const user = userEvent.setup();
        const onNav = vi.fn();
        render(_jsx(HookHarness, { onNav: onNav }));
