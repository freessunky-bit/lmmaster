...
const DEFAULT_OFF: TelemetryConfig = {
  enabled: false,
  anon_id: null,
  opted_in_at: null,
};

const ENABLED_WITH_ID: TelemetryConfig = {
  enabled: true,
...
describe("TelemetryPanel — 기본 렌더", () => {
  it("기본 비활성 상태로 렌더돼요 (aria-checked=false + statusOff)", async () => {
    ...
    expect(
      screen.getByText("screens.settings.telemetry.statusOff"),
    ).toBeTruthy();
  });
  ...
  it("description / privacyNote i18n 키가 노출돼요", async () => {
    ...
    expect(
      screen.getByText("screens.settings.telemetry.description"),
    ).toBeTruthy();
    expect(
      screen.getByText("screens.settings.telemetry.privacyNote"),
    ).toBeTruthy();
  });
  ...
  it("토글 켜면 setTelemetryEnabled(true) 호출 + UUID hint 노출", async () => {
    ...
    expect(
      screen.getByText("screens.settings.telemetry.statusOn"),
    ).toBeTruthy();
  });
  ...
  it("토글 실패 → revert + 한국어 에러 키 노출", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("screens.settings.telemetry.errors.toggleFailed"),
      ).toBeTruthy();
    });
    warnSpy.mockRestore();
  });
  ...
  it("초기 로드 실패 → loadFailed 에러 키 노출", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("screens.settings.telemetry.errors.loadFailed"),
      ).toBeTruthy();
    });
    warnSpy.mockRestore();
  });
