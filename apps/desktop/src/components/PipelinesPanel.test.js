...
function makeAudit(pipelineId, action, details = null, iso = "2026-04-28T01:23:45Z") {
    return {
...
describe("PipelinesPanel — 기본 렌더 + 토글", () => {
    ...
    it("i18n 라벨 키가 노출돼요 (이름 + 설명)", async () => {
        render(_jsx(PipelinesPanel, {}));
        await waitFor(() => {
            expect(screen.getByText("screens.settings.pipelines.pipelines.piiRedact.name")).toBeTruthy();
        });
        expect(screen.getByText("screens.settings.pipelines.pipelines.tokenQuota.name")).toBeTruthy();
        expect(screen.getByText("screens.settings.pipelines.pipelines.observability.name")).toBeTruthy();
        expect(screen.getByText("screens.settings.pipelines.pipelines.piiRedact.desc")).toBeTruthy();
    });
    it("토글 클릭 → setPipelineEnabled(false) 호출 + aria-checked flip", async () => {
        ...
        await waitFor(() => {
            expect(screen.getByTestId("pipelines-toggle-pii-redact").getAttribute("aria-checked")).toBe("false");
        });
    });
    it("토글 실패 → optimistic UI revert + 한국어 에러 키 노출", async () => {
        ...
        // 에러 메시지 — i18n 키 노출.
        await waitFor(() => {
            expect(screen.getByText("screens.settings.pipelines.errors.toggleFailed")).toBeTruthy();
        });
        warnSpy.mockRestore();
    });
    it("unknown-pipeline backend 에러 — 전용 메시지 노출", async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText("screens.settings.pipelines.errors.unknownPipeline")).toBeTruthy();
        });
        warnSpy.mockRestore();
    });
});
describe("PipelinesPanel — 감사 로그", () => {
    it("빈 로그 — empty 상태 i18n 키 노출", async () => {
        ...
        expect(screen.getByText("screens.settings.pipelines.audit.empty")).toBeTruthy();
    });
    it("entry 있을 때 — pipeline 이름 + action 라벨 + timestamp 표시", async () => {
        ...
        // action 라벨 키.
        expect(within(list).getByText("screens.settings.pipelines.audit.actionModified")).toBeTruthy();
        expect(within(list).getByText("screens.settings.pipelines.audit.actionBlocked")).toBeTruthy();
        expect(within(list).getByText("screens.settings.pipelines.audit.actionPassed")).toBeTruthy();
    });
    ...
    it("clearAuditLog 실패 → 한국어 에러 키 노출", async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText("screens.settings.pipelines.errors.clearFailed")).toBeTruthy();
        });
        warnSpy.mockRestore();
    });
...
describe("PipelinesPanel — a11y", () => {
    it("axe violations === [] (빈 로그)", async () => {
        const { container } = render(_jsx(PipelinesPanel, {}));
        await waitFor(() => {
