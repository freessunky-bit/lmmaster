...
describe("WorkbenchArtifactPanel — 기본 렌더", () => {
    it("title / description 노출", async () => {
        render(_jsx(WorkbenchArtifactPanel, {}));
        await waitFor(() => expect(getArtifactStats).toHaveBeenCalled());
        expect(screen.getByText("screens.settings.workbench.artifacts.title")).toBeTruthy();
        expect(screen.getByText("screens.settings.workbench.artifacts.description")).toBeTruthy();
    });
    it("stats 로드되면 run count + total bytes 표시", async () => {
        ...
        // run count 라벨 포함.
        await waitFor(() => {
            expect(within(panel).getByText(/screens\.settings\.workbench\.artifacts\.runCount/)).toBeTruthy();
        });
    });
    it("로딩 중이면 loading hint 노출", async () => {
        // 완료 안 된 promise.
        let resolve = () => { };
        vi.mocked(getArtifactStats).mockReturnValueOnce(new Promise((r) => {
            resolve = r;
        }));
        render(_jsx(WorkbenchArtifactPanel, {}));
        expect(screen.getByText("screens.settings.workbench.artifacts.loading")).toBeTruthy();
        resolve(EMPTY_STATS);
        await waitFor(() => expect(getArtifactStats).toHaveBeenCalled());
    });
...
describe("WorkbenchArtifactPanel — a11y", () => {
    it("axe 위반 0건", async () => {
        const { container } = render(_jsx(WorkbenchArtifactPanel, {}));
        await waitFor(() => expect(getArtifactStats).toHaveBeenCalled());
