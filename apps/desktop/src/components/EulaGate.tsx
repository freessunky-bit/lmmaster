...
export interface EulaGateProps {
  /** EULA 자체 버전 — 본 앱 버전과 별개. minor/major 변경 시 재동의 필요. */
  eulaVersion: string;
  /** 동의 후 호출 — 상위가 onboarding 진입 등의 후속 처리. */
  onAccept(): void;
  /** 동의 후 노출되는 자식 트리. */
  children: ReactNode;
}
...
function isAlreadyAccepted(version: string): boolean {
  try {
...
function persistAcceptance(version: string): void {
  try {
...
export function EulaGate({ eulaVersion, onAccept, children }: EulaGateProps) {
  const { t, i18n } = useTranslation();
  const [accepted, setAccepted] = useState<boolean>(() =>
    isAlreadyAccepted(eulaVersion),
  );
  ...
  return (
    <div
      className="eula-gate-overlay"
      role="dialog"
      aria-modal="true"
      aria-labelledby="eula-title"
      data-testid="eula-gate-dialog"
      onKeyDown={handleKeyDown}
      ref={dialogRef}
    >
      <div className="eula-gate-card">
        <header className="eula-gate-header">
          <h1 id="eula-title" className="eula-gate-title">
            {t("screens.eula.title")}
          </h1>
          <div className="eula-gate-langs" role="radiogroup" aria-label="locale">
            <button
              type="button"
              role="radio"
              aria-checked={locale === "ko"}
              className={`eula-gate-lang${locale === "ko" ? " is-active" : ""}`}
              onClick={() => setLocale("ko")}
              data-testid="eula-lang-ko"
              ref={closeBtnRef}
            >
              {t("screens.eula.lang.ko")}
            </button>
            <button
              type="button"
              role="radio"
              aria-checked={locale === "en"}
              className={`eula-gate-lang${locale === "en" ? " is-active" : ""}`}
              onClick={() => setLocale("en")}
              data-testid="eula-lang-en"
            >
              {t("screens.eula.lang.en")}
            </button>
          </div>
        </header>

        <p className="eula-gate-version">
          {t("screens.eula.version", { version: eulaVersion })}
        </p>

        <div
          className="eula-gate-body"
          ref={bodyRef}
          onScroll={handleScroll}
          tabIndex={0}
          aria-label={t("screens.eula.title")}
        ...
        {!scrolledToEnd && (
          <p
            className="eula-gate-scroll-hint"
            role="status"
            data-testid="eula-scroll-hint"
          >
            {t("screens.eula.scrollToAccept")}
          </p>
        )}

        <footer className="eula-gate-footer">
          <button
            type="button"
            className="eula-gate-decline"
            onClick={handleDeclineRequest}
            data-testid="eula-decline"
          >
            {t("screens.eula.decline")}
          </button>
          <button
            type="button"
            className="eula-gate-accept"
            onClick={handleAccept}
            disabled={!scrolledToEnd}
            data-testid="eula-accept"
          >
            {t("screens.eula.accept")}
          </button>
        </footer>
      </div>
      ...
      {confirmingDecline && (
        <div
          className="eula-gate-confirm"
          role="dialog"
          aria-modal="true"
          aria-labelledby="eula-decline-title"
          data-testid="eula-decline-confirm"
        >
          <div className="eula-gate-confirm-card">
            <h2 id="eula-decline-title" className="eula-gate-confirm-title">
              {t("screens.eula.decline.confirmTitle")}
            </h2>
            <p className="eula-gate-confirm-body">
              {t("screens.eula.decline.confirmBody")}
            </p>
            <div className="eula-gate-confirm-actions">
              <button
                type="button"
                className="eula-gate-decline"
                onClick={handleDeclineCancel}
                data-testid="eula-decline-cancel"
              >
                {t("screens.eula.decline.cancel")}
              </button>
              <button
                type="button"
                className="eula-gate-accept"
                onClick={handleDeclineExit}
                data-testid="eula-decline-exit"
              >
                {t("screens.eula.decline.exit")}
              </button>
            </div>
          </div>
        </div>
