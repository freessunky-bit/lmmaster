...
export function compareVersion(a, b) {
    const stripV = (s) => (s.startsWith("v") || s.startsWith("V") ? s.slice(1) : s);
    // pre-release / build 메타 무시 — `1.2.3-rc1+build` → `1.2.3`.
    const core = (s) => stripV(s).split(/[-+]/)[0] ?? "";
    const partsOf = (s) => {
    ...
    // 동률이면 fallback string compare (pre-release 등 차이를 안정적으로 정렬).
 ...
 */
export function cleanupOlderSkippedVersions(latest) {
    try {
...
function formatPublishedKo(iso) {
    if (!iso)
        return "";
...
export function ToastUpdate({ release, currentVersion, onSkip, onDismiss, }) {
    ...
    return (_jsxs("div", { className: "toast-update", role: "status", "aria-live": "polite", "data-testid": "toast-update", children: [_jsxs("div", { className: "toast-update-body", children: [_jsx("span", { className: "toast-update-icon", "aria-hidden": true, children: "\u25C6" }), _jsxs("div", { className: "toast-update-text", children: [_jsx("div", { className: "toast-update-title", children: t("toast.update.title", { version: release.version }) }), _jsx("div", { className: "toast-update-detail", children: t("toast.update.detail", {
                                    currentVersion,
                                    published: publishedKo,
                                }) })] })] }), _jsxs("div", { className: "toast-update-actions", children: [_jsx("button", { type: "button", className: "onb-button onb-button-primary toast-update-cta", onClick: handleViewUpdate, "data-testid": "toast-update-view", disabled: !release.url, children: t("toast.update.view") }), _jsx("button", { type: "button", className: "onb-button onb-button-ghost", onClick: handleSkip, "data-testid": "toast-update-skip", children: t("toast.update.skip") }), _jsx("button", { type: "button", className: "toast-update-close", onClick: onDismiss, ref: closeBtnRef, "aria-label": t("toast.update.close") ?? undefined, "data-testid": "toast-update-close", children: "\u2715" })] })] }));
}
