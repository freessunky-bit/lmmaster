...
import { ToastUpdate, cleanupOlderSkippedVersions, compareVersion, } from "./ToastUpdate";
...
describe("ToastUpdate", () => {
    it("새 버전 정보 + 한국어 날짜 포맷으로 렌더", () => {
        ...
        render(_jsx(ToastUpdate, { release: FIXTURE_RELEASE, currentVersion: "1.1.0", onSkip: onSkip, onDismiss: onDismiss }));
        // i18n key + opts mock — JSON stringify 안에 version 포함.
        expect(screen.getByTestId("toast-update")).toBeTruthy();
        expect(screen.getByText(/toast\.update\.title.*"version":"1\.2\.0"/)).toBeTruthy();
    ...
    it("이번 버전 건너뛰기 → onSkip 호출 + localStorage 저장", async () => {
        ...
        expect(globalThis.localStorage.getItem("lmmaster.update.skipped.1.2.0")).toBe("true");
    });
    ...
    it("X 버튼 → onDismiss 호출 (localStorage 미저장)", async () => {
        ...
        expect(globalThis.localStorage.getItem("lmmaster.update.skipped.1.2.0")).toBeNull();
    });
    ...
    it("같은 version이 localStorage에 skipped면 마운트 즉시 onDismiss + null 렌더", async () => {
        ...
        const onSkip = vi.fn();
        const onDismiss = vi.fn();
        const { container } = render(_jsx(ToastUpdate, { release: FIXTURE_RELEASE, currentVersion: "1.1.0", onSkip: onSkip, onDismiss: onDismiss }));
        // 마운트 즉시 onDismiss 호출.
        await waitFor(() => {
            expect(onDismiss).toHaveBeenCalled();
        });
    ...
    it("URL이 빈 문자열이면 '업데이트 보기' 버튼이 disabled", () => {
        const onSkip = vi.fn();
        const onDismiss = vi.fn();
        const release = { ...FIXTURE_RELEASE, url: "" };
        render(_jsx(ToastUpdate, { release: release, currentVersion: "1.1.0", onSkip: onSkip, onDismiss: onDismiss }));
        const btn = screen.getByTestId("toast-update-view");
    ...
    it("a11y — 위반 0건 (axe)", async () => {
        const { container } = render(_jsx(ToastUpdate, { release: FIXTURE_RELEASE, currentVersion: "1.1.0", onSkip: vi.fn(), onDismiss: vi.fn() }));
...
describe("cleanupOlderSkippedVersions (Phase 8'.a.2)", () => {
    ...
    it("새 버전 도착 시 이전(낮은) 버전 키 제거", () => {
        ...
        expect(globalThis.localStorage.getItem("lmmaster.update.skipped.1.0.0")).toBeNull();
        expect(globalThis.localStorage.getItem("lmmaster.update.skipped.1.0.5")).toBeNull();
    });
    it("같은 버전 또는 더 높은 버전 키는 보존 (downgrade 안전)", () => {
        ...
        expect(globalThis.localStorage.getItem("lmmaster.update.skipped.2.0.0")).toBe("true");
    });
    it("다른 prefix 키는 건드리지 않음", () => {
        ...
        expect(globalThis.localStorage.getItem("lmmaster.update.skipped.1.0.0")).toBeNull();
        expect(globalThis.localStorage.getItem("lmmaster.unrelated.value")).toBe("keep");
        expect(globalThis.localStorage.getItem("other.app.skipped.1.0.0")).toBe("keep");
    });
    it("ToastUpdate 마운트 시 자동 정리 — 1.0.0 skipped + 1.2.0 도착 → 1.0.0 키 제거", async () => {
        globalThis.localStorage.setItem("lmmaster.update.skipped.1.0.0", "true");
        const { container: _ } = render(_jsx(ToastUpdate, { release: FIXTURE_RELEASE, currentVersion: "1.1.0", onSkip: vi.fn(), onDismiss: vi.fn() }));
        await waitFor(() => {
            expect(globalThis.localStorage.getItem("lmmaster.update.skipped.1.0.0")).toBeNull();
        });
    });
