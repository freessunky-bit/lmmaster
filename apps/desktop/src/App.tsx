...
import { getLastScan, onScanSummary, type ScanSummary } from "./ipc/scanner";
...
function MainShell({
  ...
  useShortcutsHotkey({
    ...
    onNav: (navKey) => {
      if ((NAV_KEYS as readonly string[]).includes(navKey)) {
        setActiveNav(navKey as NavKey);
      }
    },
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) console.warn("get_gateway_status failed:", e);
      }
      ...
      try {
        ...
        if (!cancelled) console.warn("get_last_scan failed:", e);
      }
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      if (
        ...
        (NAV_KEYS as readonly string[]).includes(detail)
      ) {
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <StatusPill
          ...
          ariaLabel={gw.error ?? undefined}
        />
      </aside>
      <main className="content">
        <header className="topbar">
          <div className="title">{t(`nav.${activeNav}`)}</div>
        </header>
        <WorkspaceRepairBanner />
        {activeNav === "catalog" ? (
          <CatalogPage />
        ) : activeNav === "keys" ? (
          <ApiKeysPanel />
          ...
          <Home
            gw={gw}
            scanSummary={scan?.summary_korean}
            onPickModel={(modelId) => {
              try {
...
function mapGatewayStatus(s: GatewayState["status"]): PillStatus {
  switch (s) {
    case "listening":
      return "listening";
    case "failed":
