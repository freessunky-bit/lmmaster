...
- **픽토그램 정책 (Phase 14', 2026-05-03)** — *컬러 이모지(🤗🔥🔞⚠️ 등) 직접 사용 금지*.
  - **표준 라이브러리**: `lucide-react` (이미 `apps/desktop`에 도입). currentColor + monochrome stroke + 24px grid + named import (tree-shaking).
  - **사용 패턴**: `<Sparkles size={14} aria-hidden="true" />` + 텍스트 라벨 별도 `<span>`. button은 `display: inline-flex; align-items: center; gap: var(--space-2)`로 정렬.
  - **자체 픽토그램**: `apps/desktop/src/components/Brand.tsx` — LMmaster 공식 마크 + lockup. 둥근 사각 컨테이너 + M zigzag. drop-shadow 글로우는 `--primary-a-2/3` 토큰만.
  - **외부 brand 마크 예외**: HuggingFace / GitHub 등 *외부 서비스 brand asset*은 brand recognition을 위해 컬러 SVG 사용 OK. `HuggingFaceMark.tsx`처럼 자체 컴포넌트 작성 + 사용 위치 limited (해당 서비스 진입점만).
  - **이모지 in i18n 금지**: `ko.json`/`en.json`에 직접 이모지(🔥/🔞 등) 인라인 X. 라벨은 plain text — 컴포넌트 측에서 lucide icon prepend.
  - **신규 픽토그램 필요 시**: 먼저 lucide-react에 있는 것 재사용. 없으면 자체 SVG 컴포넌트 (currentColor + 24x24 viewBox + stroke 1.5~2 + linecap round). 위치는 `apps/desktop/src/components/<Mark>.tsx` + 별개 CSS.
